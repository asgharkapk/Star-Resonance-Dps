...
namespace StarResonanceDpsAnalysis.Plugin
{
    ...
    public class AppConfig
    {
        ...
        public static RenderContent.ContentOffset ProgressBarImage
        {
            get
            {
                int x = 48 -32 +6 +6;// shifted left by 32
                if(dpi==1)
                {
                    x = 40 -32 +6 +6;// shifted left by 32
                }

                return new RenderContent.ContentOffset { X = x, Y = 0 };
            }
        }
        public static RenderContent.ContentOffset ProgressBarNmae = new RenderContent.ContentOffset { X = 70 -32 +6, Y = 1 };// shift left by 32
        public static RenderContent.ContentOffset ProgressBarHarm
        {
            get
            {
                int x = ProgressBarProportion.X - 25 -32 +6;// shift left by 32
                switch(dpi)
                {
                    case 1:
                        x = -22 -32 +6;// shift left by 32
                        break;
                    case (float)1.25:
                        x = -28 -32 +6;// shift left by 32
                        break;
                    case (float)1.5:
                        x = ProgressBarProportion.X - 30 -32 +6;// shift left by 32
                        break;
                    case (float)1.75:
                        x = ProgressBarProportion.X - 38 -32 +6;// shift left by 32
                        break;
                    case 2:
                        x = ProgressBarProportion.X - 42 -32 +6;// shift left by 32
                        break;
                }
                if(dpi==1)
                {
