...
namespace StarResonanceDpsAnalysis.Plugin
{
    ...
    public class AppConfig
    {
        ...
        public static RenderContent.ContentOffset ProgressBarImage
        {
            get
            {
                int x = 16 + 6;// shifted left by 32
                if(dpi==1)
                {
                    x = 8 + 6;// shifted left by 32
                }

                return new RenderContent.ContentOffset { X = x, Y = 0 };
            }
        }
        public static RenderContent.ContentOffset ProgressBarNmae = new RenderContent.ContentOffset { X = 70 -30, Y = 1 };// shift left by 32
        public static RenderContent.ContentOffset ProgressBarHarm
        {
            get
            {
                int x = ProgressBarProportion.X - 25 -30;// shift left by 32
                switch(dpi)
                {
                    case 1:
                        x = -22 -30;// shift left by 32
                        break;
                    case (float)1.25:
                        x = -28 -30;// shift left by 32
                        break;
                    case (float)1.5:
                        x = ProgressBarProportion.X - 30 -30;// shift left by 32
                        break;
                    case (float)1.75:
                        x = ProgressBarProportion.X - 38 -30;// shift left by 32
                        break;
                    case 2:
                        x = ProgressBarProportion.X - 42 -30;// shift left by 32
                        break;
                }
                if(dpi==1)
                {
